...
$(document).ready(function () {
   ...
   **********************************/
    ...
    function set_meal_editor_data(meal_id)
    {
        ...
        {
            ingredients += '<li class="flex-ingredient-item"><div class="ingredient" id="ingredient_' + i + '">' + current_meal.ingredients[i] + '</div><div class="remove_ingredient_button" id="button_1">x</div></li>';
    ...
    function setup_meal_onclick_function()
    {
        for (var i = 0; i < meals.length; i++)
        {
            document.getElementById('drag_' + meals[i].id).onclick = (function (current_i) { return function () { set_meal_editor_data(current_i); } })(i);
        }
    }
        ...
        for (var i = 0; i < 35; i++) 
        {
            if (i >= first_day && day <= number_of_days)
            {
                if ((first_day == 5 || first_day == 6) && (number_of_days == 31) && (day == 24))
                    calendar_day_squares += '<li class="flex_calendar_body_item_half_day"><div id="calendar_day_div_' + i + '" ondrop="drop(event)" ondragover="allow_drop(event)">' + day + '</div><div id="calendar_day_div_' + i + '" ondrop="drop(event)" ondragover="allow_drop(event)">31</div></li>';
                else if ((first_day == 6) && (number_of_days >= 30) && (day == 23))
                    calendar_day_squares += '<li class="flex_calendar_body_item_half_day"><div id="calendar_day_div_' + i + '" ondrop="drop(event)" ondragover="allow_drop(event)">' + day + ' </div><div id="calendar_day_div_' + i + '" ondrop="drop(event)" ondragover="allow_drop(event)">30</div></li>';
                else
